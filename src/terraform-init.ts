...
export async function waitForProcess(logFile: string, outputWindow: vscode.OutputChannel): Promise<void> {
    const lines: string[] = [];
...
export async function runTerraformInit(document: vscode.TextDocument): Promise<void> {
    ...
    try {
        ...
        if (logContent.includes("Terraform has been successfully initialized") ||
    ...
    if (!initSucceeded) {
        outputWindow.appendLine("Error: Initialization did not complete successfully. Check the log for details.");
